...
#iris.data is the real values for features
#iris.target [0,1,2] is Iris species
...
# Get dataset with only the first two attributes, sepal width and sepal length
X, y = X_iris[:, :2], y_iris
...
#len means the length, so it is 3 here
#range(3) means (0, 1, 2)
...
#SGDClassifier is used to implement linear classification
from sklearn.linear_model import SGDClassifier
clf = SGDClassifier()
#fit receives the training data and the classes, and builds the classifier
...
#with three values, we can draw a line
#ie. -17.62477802-28.53692691 * x1 + 15.05517618 * x2 = 0
#if the above equation is greater than zero, then the point is above the decision boundary
...
for i in [0, 1, 2]:
    ...
    #error here need plt.
    #plt.sca means to set the current axes instance to axes[i]
    plt.sca(axes[i])
    plt.scatter(X_train[:, 0], X_train[:, 1], c=[colors[x] for x in y_train])
